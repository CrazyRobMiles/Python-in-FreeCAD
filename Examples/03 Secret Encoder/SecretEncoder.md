...
![Picture of the Secret Encoder](/Examples/03%20Secret%20Encoder/SecretEncoder.png)

Not super secret, but quite fun. Take the 3D printed panels and hold them on top of each other. Then look through to read the secret message written on them. Each panel is different, only when they are put together is the message revealed. You can have as many panels as you like, although one panel might not be the best idea if you want to keep something secret. 
## Setting Values

Change these values to set the text you want to encode and the size and number of the panels. 
...
The program produces two versions of the panel which are used for the preview display. They are both the same. It is quite long and also includes designs for each character on a 5 pixel high font. 
