...
![Lots of boxes, round and square](/Examples/10%20Lots%20of%20Boxes/LotsOfBoxes.png)

Since we have a loop that can make lots of round boxes, we might as well use it to make a lot of square ones too. 
...
doc = App.newDocument("Lots of Boxes")
...
for size in range(100,20,-10):
	...
	box,lid = makeBoxAndLid(size,size,height,lid_height,wall_thickness)

	box.translate(App.Vector(170,y,0))
	lid.translate(App.Vector(170,y,0))

	frame_obj = doc.addObject("Part::Feature", f"Square Box {size}")
	frame_obj.Shape = box

	frame_obj = doc.addObject("Part::Feature", f"Square Lid {size}")
	frame_obj.Shape = lid
