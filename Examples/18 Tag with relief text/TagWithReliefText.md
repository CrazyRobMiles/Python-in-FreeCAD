# Tag With Relief Text
![A line drawing of a path which contains a rounded element at one end. The vertices and the edges are labeled.](/Examples/18%20Tag%20with%20relief%20text/TagWithReliefText.png)

This shows how we can create create a string shape and then extrude this to produce text which can then be fused with the tag. This version can be printed in a single colour 3D printer. The text is raised above the surface of the tag. The variable **tag_text** holds the string to be printed on the tag. Use upper case letters if you want the text to always fit the tag. If you want to use lower case ones, increase the size of the text margins to taste. 
```python
# Extruded Panel
