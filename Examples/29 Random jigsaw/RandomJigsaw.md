# Random Jigsaw
![FreeCAD showing a mosaic image cut into jigsaw pieces with randomly-positioned tabs](/Examples/29%20Random%20jigsaw/randomjigsaw.png)

This is a variation of the [Jigsaw Puzzle](/Examples/28%20Jigsaw%20Puzzle/JigsawPuzzle.md) where each tab is placed at a random position along its piece edge rather than always at the centre. This makes neighbouring pieces easier to tell apart and produces a more authentic jigsaw feel.

The tab size stays fixed; only its offset along the edge varies. `MIN_LEAD_FRAC` controls how close a tab can get to the edge of a piece, preventing it from running off either end.
...
* **BASE_THICKNESS_MM** — height of the base plate in mm
...
* **KERF_MM** — gap between printed pieces in mm
* **RANDOM_SEED** — controls tab directions and positions; change for a different pattern
* **MIN_LEAD_FRAC** — minimum fraction of the piece span that must appear as a straight track before and after each tab; prevents tabs from running off the edge of a piece
